...
当前仓库支持两种 go-live 门禁模式：

- 演示/试运行模式（默认）：`GO_LIVE_ENFORCE_PROD_POLICY=0`
- 生产发布模式：`GO_LIVE_ENFORCE_PROD_POLICY=1`

说明：

- 当为 `0` 时，`prod` 策略检查结果会记录在报告中，但不会作为阻塞项。
- 当为 `1` 时，`prod` 策略检查会成为硬门禁，不通过则 `go_live_preflight` 直接 `BLOCK`。

建议：

- 日常联调、课堂演示、阶段验收先用 `0`。
- 正式上线窗口切换到 `1`，并连续通过多轮稳定性验收后再发布。
...
- `docs/ops/go_live_bundle_cron_cn.md`
