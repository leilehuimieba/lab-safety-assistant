...
def run_data_sources_check(repo_root: Path, errors: list[str]) -> None:
    cmd = [
        sys.executable,
        str(repo_root / "scripts" / "validate_data_sources.py"),
        "--repo-root",
        str(repo_root),
        "--quiet",
    ]
    ...
    if completed.returncode != 0:
        errors.append(
            "data_sources schema 检查未通过。"
            + (f"\n  detail: {completed.stdout.strip()}" if completed.stdout else "")
        )


def run_release_review_log_check(repo_root: Path, errors: list[str]) -> None:
    cmd = [
        sys.executable,
        str(repo_root / "scripts" / "validate_release_review_log.py"),
        "--repo-root",
        str(repo_root),
        "--quiet",
    ]
...
def run_release_policy_schema_check(repo_root: Path, errors: list[str]) -> None:
    cmd = [
        sys.executable,
        str(repo_root / "scripts" / "validate_release_policy_schema.py"),
        "--repo-root",
        str(repo_root),
        "--quiet",
    ]
