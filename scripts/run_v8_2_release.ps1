...
$Manifest = "data_sources\web_seed_urls_v8_2_candidates.csv"
$PrefetchDir = "artifacts\web_seed_v8_2_prefetch"
...
$StatusCsv = "data_sources\web_seed_urls_v8_2_prefetch_status.csv"
$ReportMd = "docs\pipeline\web_seed_v8_2_prefetch_report.md"
$AssignmentCsv = "data_sources\web_seed_v8_2_task_assignment.csv"
$BundleDir = "artifacts\import_bundle_v8_2"
$ReleaseDir = "release_exports\v8.2"
$CuratedCsv = "knowledge_base_curated.csv"
$BaseReleaseCsv = "release_exports\v8.1\knowledge_base_import_ready.csv"
...
if (-not $SkipFetch) {
  Write-Host "=== Step 1/6: Fetch V8.2 web seeds (skill mode) ===" -ForegroundColor Cyan
  ...
  if ($LASTEXITCODE -ne 0) { throw "V8.2 web fetch failed." }
}
...
Write-Host "=== Step 2/6: Generate V8.2 prefetch status ===" -ForegroundColor Cyan
& $PythonExe scripts\generate_web_prefetch_status.py `
  --manifest $Manifest `
  --fetch-results $FetchResults `
  --output $StatusCsv
if ($LASTEXITCODE -ne 0) { throw "Generate V8.2 prefetch status failed." }
...
if ($LASTEXITCODE -ne 0) { throw "Generate V8.2 prefetch report failed." }
...
Write-Host "=== Step 5/6: Build import-ready bundle (curated + v8.2 + previous v8.1) ===" -ForegroundColor Cyan
$buildArgs = @(
  "scripts\build_import_ready_bundle.py",
  "--output-dir", $BundleDir,
  "--source", "curated=$CuratedCsv",
  "--source", "v8_2_web_rewritten=$KbRewrittenCsv"
)
if (Test-Path $BaseReleaseCsv) {
  $buildArgs += @("--source", "v8_1_previous_release=$BaseReleaseCsv")
}
& $PythonExe @buildArgs
if ($LASTEXITCODE -ne 0) { throw "Build V8.2 import bundle failed." }

Write-Host "=== Step 6/6: Export V8.2 release directory ===" -ForegroundColor Cyan
...
Copy-Item $Manifest "$ReleaseDir\web_seed_urls_v8_2_candidates.csv" -Force
Copy-Item $StatusCsv "$ReleaseDir\web_seed_urls_v8_2_prefetch_status.csv" -Force
Copy-Item $AssignmentCsv "$ReleaseDir\web_seed_v8_2_task_assignment.csv" -Force
Copy-Item $ReportMd "$ReleaseDir\web_seed_v8_2_prefetch_report.md" -Force
Copy-Item $KbCsv "$ReleaseDir\knowledge_base_web_v8_2.csv" -Force
Copy-Item $KbRewrittenCsv "$ReleaseDir\knowledge_base_web_v8_2_rewritten.csv" -Force
Copy-Item $RewriteLogCsv "$ReleaseDir\rewrite_log_v8_2.csv" -Force
if (Test-Path "$PrefetchDir\run_report.json") {
  Copy-Item "$PrefetchDir\run_report.json" "$ReleaseDir\web_seed_v8_2_run_report.json" -Force
}
...
- source_priority: curated > v8_2_web_rewritten > v8_1_previous_release(optional)

## Included files

1. knowledge_base_import_ready.csv
2. import_bundle_report.json / import_bundle_report.md
3. web_seed_urls_v8_2_candidates.csv
4. web_seed_urls_v8_2_prefetch_status.csv
5. web_seed_v8_2_prefetch_report.md
6. web_seed_v8_2_task_assignment.csv
7. knowledge_base_web_v8_2.csv
8. knowledge_base_web_v8_2_rewritten.csv
9. rewrite_log_v8_2.csv
10. web_seed_v8_2_run_report.json (if available)
"@
$releaseNote | Set-Content -Path "$ReleaseDir\README.md" -Encoding UTF8

Write-Host ""
Write-Host "=== V8.2 release pipeline done ===" -ForegroundColor Green
