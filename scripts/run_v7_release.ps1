...
$Manifest = "data_sources\web_seed_urls_v7_candidates.csv"
$PrefetchDir = "artifacts\web_seed_v7_prefetch"
...
$StatusCsv = "data_sources\web_seed_urls_v7_prefetch_status.csv"
$ReportMd = "docs\pipeline\web_seed_v7_prefetch_report.md"
$AssignmentCsv = "data_sources\web_seed_v7_task_assignment.csv"
...
if (-not $SkipFetch) {
  ...
  & $PythonExe scripts\web_ingest_pipeline.py `
    ...
    --max-chars $MaxChars `
    --overlap $Overlap
  if ($LASTEXITCODE -ne 0) { throw "V7 web fetch failed." }
}
...
Write-Host "=== Step 2/6: Generate V7 prefetch status ===" -ForegroundColor Cyan
& $PythonExe scripts\generate_web_prefetch_status.py `
  --manifest $Manifest `
  --fetch-results $FetchResults `
  --output $StatusCsv
if ($LASTEXITCODE -ne 0) { throw "Generate V7 prefetch status failed." }
...
& $PythonExe scripts\generate_web_prefetch_report.py `
  ...
  --batch-name "web_seed_v7"
if ($LASTEXITCODE -ne 0) { throw "Generate V7 prefetch report failed." }
...
Copy-Item $Manifest "$ReleaseDir\web_seed_urls_v7_candidates.csv" -Force
Copy-Item $StatusCsv "$ReleaseDir\web_seed_urls_v7_prefetch_status.csv" -Force
Copy-Item $AssignmentCsv "$ReleaseDir\web_seed_v7_task_assignment.csv" -Force
Copy-Item $ReportMd "$ReleaseDir\web_seed_v7_prefetch_report.md" -Force
Copy-Item $KbCsv "$ReleaseDir\knowledge_base_web_v7.csv" -Force
Copy-Item $KbRewrittenCsv "$ReleaseDir\knowledge_base_web_v7_rewritten.csv" -Force
Copy-Item $RewriteLogCsv "$ReleaseDir\rewrite_log_v7.csv" -Force
if (Test-Path "$PrefetchDir\run_report.json") {
  Copy-Item "$PrefetchDir\run_report.json" "$ReleaseDir\web_seed_v7_run_report.json" -Force
}
...
3. web_seed_urls_v7_candidates.csv
4. web_seed_urls_v7_prefetch_status.csv
5. web_seed_v7_prefetch_report.md
6. web_seed_v7_task_assignment.csv
7. knowledge_base_web_v7.csv
8. knowledge_base_web_v7_rewritten.csv
9. rewrite_log_v7.csv
10. web_seed_v7_run_report.json (if available)
"@
$releaseNote | Set-Content -Path "$ReleaseDir\README.md" -Encoding UTF8

Write-Host ""
Write-Host "=== V7 release pipeline done ===" -ForegroundColor Green
