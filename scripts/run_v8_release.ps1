...
if (-not $SkipFetch) {
  ...
  & $PythonExe scripts\web_ingest_pipeline.py `
    ...
    --max-chars $MaxChars `
    --overlap $Overlap
  if ($LASTEXITCODE -ne 0) { throw "V8 web fetch failed." }
}
...
& $PythonExe @buildArgs
if ($LASTEXITCODE -ne 0) { throw "Build V8 import bundle failed." }
...
3. web_seed_urls_v8_candidates.csv
4. web_seed_urls_v8_prefetch_status.csv
5. web_seed_v8_prefetch_report.md
6. web_seed_v8_task_assignment.csv
7. knowledge_base_web_v8.csv
8. knowledge_base_web_v8_rewritten.csv
