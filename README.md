...
- Weekly ops now includes release-fix P0 stats: created/closed/open/open>=7d task counts.
...
- Fix-task issue sync: `python scripts/sync_release_fix_plan_issues.py --repo-root . --only-priority P0 --assign-from-owner` (sync P0 tasks to GitHub Issues with owner->assignee mapping and fallback).
- Sync reports: `docs/ops/release_fix_plan_sync_report.json` and `docs/ops/release_fix_plan_sync_report.md`.
- Overdue escalation: `python scripts/escalate_release_fix_overdue.py --repo-root . --priority P0` (adds overdue/p1 labels and daily follow-up comments).
- Overdue reports: `docs/ops/release_fix_overdue_report.json` and `docs/ops/release_fix_overdue_report.md`.
